...
def Test_mimic3(model, model_path, device, voc_size):
    ...
    tic, result, sample_size = time.time(), [], round(7495 * 0.8)
    for _ in range(10):
        test_sample = np.random.choice(7495, sample_size, replace=True)
...
def test_Eval(model, test_sample, voc_size):
    ...
    file = open('./dataset/data_eval_7495.pkl', 'rb')
    #print(test_sample)
    step=-1
    for idx in test_sample: 
        ...
        llprint("\reval step: {} / {}".format(step, len(test_sample)))  

    ddi_rate = ddi_rate_score(smm_record, path="./data/ddi_A_final_4.pkl")
...
def Eval_mimic3(model, voc_size):
    ...
    ja, prauc, avg_p, avg_r, avg_f1 = [[] for _ in range(5)] 
    med_cnt, visit_cnt = 0, 0
    file = open('./dataset/data_test_7496.pkl', 'rb')

    
    for step in range(7496):  
        input = pickle.load(file)
        y_gt, y_pred, y_pred_prob, y_pred_label = [], [], [], [] 
        ...
        llprint("\reval step: {} / {}".format(step, 7496))
    file.close()
...
def Train_mimic3(model, device, voc_size, args):
    ...
    for epoch in range(EPOCH):
        ...
        model.train()
        loss_list = []
        file = open('./dataset/data_train_29982.pkl', 'rb')
        
        for step in range(29982):
            input = pickle.load(file)
...
失
            ...
            current_ddi_rate = ddi_rate_score(
                [[y_label]], path="./data/ddi_A_final_4.pkl"
            )
            ...
            llprint("\rtraining step: {} / {}".format(step, 29982))
