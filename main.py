...
parser.add_argument("--resume_path", type=str, default='./saved/Epoch_20_TARGET_0.06_JA_0.5317_DDI_0.06831.model', help="resume path")
...
parser.add_argument("--kp", type=float, default=2.5, help="coefficient of P signal")
...
def main():
    ...
    # load model
    voc_size, ddi_adj, ehr_adj, patient_num  = read_data_inf('mimic3')
    # print('patient_num', patient_num)
    model = TimeRec_GCN(voc_size, patient_num, args.emb_dim, args.dp, args.dp, args.temporal_information_importance, ehr_adj, ddi_adj, device).to(device)
